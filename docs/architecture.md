...
| 位置 | 职责 |
| --- | --- |
| `src/features/home/` | 三幕全屏首页、原山景逐帧背景、资源调度 |
...
首页恢复自然滚动改版前的三幕固定舞台：品牌、理念、海面行动区随滚动进度依次出现。装饰Canvas按进度读取原片图片帧，不下载或转换MP4。最多4并发、24个位图，隐藏暂停、退出释放；弱网、触屏、小屏、减少动态效果与能力不足保留原版静态阅读降级。详见[首页性能](home-performance.md)。
