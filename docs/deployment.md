...
- 验证接口 `POST /api/invite`：平台与带会话的生成请求共用一个按 IP 计数的限额，每 60 秒最多 2 次；正常首次验证与随后的生成恰好使用 2 次，错误验证也计数。无会话生成由后端直接拒绝，不调用模型；应用实例内每客户端 10 分钟最多 5 次、合计最多 60 次（成功也计数）。自托管未接可信代理时共用客户端桶。无数据库意味着这不是跨实例的精确计次或一次性邀请码。
...
当前 Hobby 套餐支持 IP / JA4 计数键，不支持按路径分别计数。本项目采用单条 IP 规则，将邀请码验证和携带会话 Cookie 的生成请求合并计数。参见 [Vercel 官方限额](https://vercel.com/docs/security/vercel-waf/rate-limiting#limits)。
