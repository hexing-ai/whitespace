...
最终生产部署 `5d1aa36` 的[线上性能验收](https://github.com/hexing-ai/whitespace/actions/runs/34235501254)通过：冷缓存1458ms可读、热缓存976ms、受限场景1935ms；滚动RAF间隔P95≤16.8ms，冷缓存慢滚曾记录单次66.7ms间隔，不宣称每一帧恒定60fps。无观测到的滚动长任务，CLS、动画媒体请求和静止应用RAF均为0。数据来自GitHub Actions独立网络；本机预览工具打开Vercel曾超时，未据此保证所有地区访问速度。
