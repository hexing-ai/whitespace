...
import { useVideoScroll } from "./use-video-scroll";
import "./home.css";
...
export default function CinematicHome({ brand, help, enter }: { brand: ReactNode; help: () => void; enter: (example: boolean) => void }) {
  ...
  function closeMenu(action?: () => void) {
    ...
    closeTimer.current = setTimeout(() => { menu.current?.close(); setMenuActive(false); action?.(); }, matchMedia("(prefers-reduced-motion: reduce)").matches ? 0 : 500);
  }
  ...
  function jump(chapter: number) {
    ...
    if (element.dataset.mode === "cinematic") {
      const top = element.getBoundingClientRect().top + window.scrollY;
      window.scrollTo({ top: top + homeConfig.chapters[chapter] * (element.offsetHeight - innerHeight), behavior: reduced ? "instant" : "smooth" });
    } else section.scrollIntoView({ behavior: reduced ? "instant" : "smooth" });
  ...
  return <div ref={root} className="cinematic-home" data-mode="static" data-video-state="static" data-renderer="poster">
    ...
    <div className="home-scene">
      ...
      <section className="home-chapter home-brand" aria-label="认识留白" data-visible="true">
        ...
        <button className="home-down home-circle" aria-label="向下了解" onClick={() => jump(1)}><ArrowRight size={18} aria-hidden="true" /></button>
      </section>
      <section className="home-chapter home-value" aria-labelledby="home-value-title" data-visible="true">
      ...
      <section className="home-chapter home-action" aria-labelledby="home-action-title" data-visible="true">
        <div className="home-copy">
      ...
      <nav className="home-navigation" aria-label="首页章节" hidden>
        <button className="home-circle home-chapter-down" aria-label="前往开始行动" onClick={() => jump(2)}><ArrowDown size={18} aria-hidden="true" /></button>
        <div className="home-dots">{["认识留白", "理解价值", "开始行动"].map((label, index) => <button key={label} data-chapter-link={index} aria-label={label} onClick={() => jump(index)}><span /></button>)}</div>
        <button className="home-circle home-chapter-up" aria-label="返回首屏" onClick={() => jump(0)}><ChevronUp size={16} aria-hidden="true" /></button>
      </nav>
    </div>
