...
try{
 ...
 for(const scenario of ['cold','warm','constrained']){
  ...
  if(scenario==='cold') for(const [name,position,chapter] of [['home',0,'1'],['value',.45,'2'],['action',.9,'3']]) {
   await home.evaluate((el,p)=>scrollTo(0,(el.clientHeight-innerHeight)*p),position);
   await expect(home).toHaveAttribute('data-chapter',chapter);await page.waitForTimeout(1600);
