...
const output = 'test-results/home-natural'; await mkdir(output, { recursive: true });
...
try {
  ...
  for (const scenario of ['cold', 'warm', 'constrained']) {
    ...
    await page.addInitScript(() => {
      ...
      const probe = window.homeProbe = { rafs: 0, samples: [], longTasks: [], active: false, lcp: 0, cls: 0 };
      ...
      probe.start = () => { probe.samples = []; probe.longTasks = []; probe.active = true; const tick = t => { if (!probe.active) return; probe.samples.push(t); original(tick); }; original(tick); };
    });
    ...
    for (const [name, delta, count, interval] of [['slow', 70, 28, 80], ['fast', 470, 6, 65], ['reverse', -190, 18, 65]]) {
      ...
      const data = await page.evaluate(() => { const p = window.homeProbe; p.active = false; return { samples: p.samples, longTasks: p.longTasks, y: scrollY }; });
      const gaps = data.samples.slice(1).map((n, i) => n - data.samples[i]);
      item.paths.push({ name, sampledFrames: gaps.length, frameGapP95Ms: p95(gaps), frameGapMaxMs: Math.max(0, ...gaps), longTaskMaxMs: Math.max(0, ...data.longTasks), finalScrollY: data.y });
    }
    item.animationRequests = resources.filter(u => /\.(mp4|webp)(\?|$)|\/media\/frames\//.test(u)).length;
    ...
    item.passed = errors.length === 0 && workers === 0 && item.animationRequests === 0 && item.idleApplicationRafs === 0 && item.cls < .1 && item.navigationToReadableMs < (constrained ? 10000 : 3000) && item.paths.every(p => p.sampledFrames > 10 && p.frameGapP95Ms <= (constrained ? 100 : 50) && p.longTaskMaxMs <= 200);
    await page.close();
  }
